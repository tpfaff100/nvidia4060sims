...
static float g_honkLevel=0.5f;   /* set from main before buildTraffic  */
...
static Pass makeCar(float tPass,int lane,float sr)
{
    ...
    /* random lone honker — scales with honk level                       */
    if(frand()<fminf(0.40f,0.08f*g_honkLevel)) giveHorn(p,nat,frand()<0.2f);
...
/*
 * honkLevel  0.0 = silent roads, no honking at all
 *            0.5 = polite but impatient (default)
 *            1.0 = full rush-hour rage
 *            2.0 = absolute gridlock chaos
 * Passed in from main via command-line arg 4.
 */
static int buildTraffic(Pass*out,float sr,float honkLevel)
{
    /* clamp to sane range                                               */
    ...
    float pTailgateHonk = fminf(0.95f, 0.65f * honkLevel);  /* was 0.65 */
    float pRandomHonk   = fminf(0.40f, 0.08f * honkLevel);  /* was 0.08 */
    float pAngry        = fminf(0.60f, 0.25f * honkLevel);  /* lean-on  */
    float gapThresh     = 1.2f + 0.8f * honkLevel;          /* s: wider = more triggers */
    ...
    while(n<MAX_PASS-40 && t<DUR_SEC-6.f){
        ...
        if(frand()<0.45f){
            ...
            for(int j=0;j<nf&&n<MAX_PASS-40;j++){
                ...
                if(gap<gapThresh && frand()<pTailgateHonk && !f.hornN){
                    float rn2=frand();
                    Nation nat=(rn2<0.5f)?US:(rn2<0.75f)?JP:UK;
                    giveHorn(f,nat,frand()<pAngry);
                    nHonks++;
                }
                out[n++]=f;
            }
...
int main(int argc,char**argv)
{
    ...
    float honkLevel=(argc>4)?(float)atof(argv[4]):0.5f;
    g_honkLevel=honkLevel;
    srand(seed);

    printf("GPU Freeway v2 | RUSH HOUR + impatient drivers | 5 min stereo\n");
    printf("SEED: %u  honk: %.1f\n",seed,honkLevel);
    printf("(rerun 'freeway2 out.wav %d %u %.1f' to reproduce)\n\n",
           sr,seed,honkLevel);
    ...
    int np=buildTraffic(hp,(float)sr,honkLevel);
