...
static int compose(Composer&C,Note*notes,char*report,int length=5)
{
    C.notes=notes; C.n=0;
    ...
    p+=snprintf(p,(size_t)(report_end-p),"Key: %s minor   Tempo: %d BPM   Length: %d/10\nForm: ",
               NOTE_N[C.key%12],C.tempo,length);
...
int main(int argc,char**argv)
{
    ...
    printf("SEED: %u   LENGTH: %d/10\n",seed,length);
    printf("(rerun './discogen out.wav %d %u %d')\n\n",sr,seed,length);
    ...
    Composer C={};
    char report[512];
    int nNotes=compose(C,notes,report,length);
    printf("%s",report);
    printf("Notes: %d\n\n",nNotes);
