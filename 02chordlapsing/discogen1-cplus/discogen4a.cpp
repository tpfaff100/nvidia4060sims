...
static void percBar(Composer&C,float barBeat,float energy,int fill,float kickBoost=1.0f){
    for(int b=0;b<4;b++){
        float kvel=std::min(1.0f,(0.95f*energy+0.2f)*kickBoost);
...
static void genPerc(Patch&P, float kickBoost=1.0f){
    ...
    /* ── KICK DRUM (notes on MIDI 24, played by osc 0-2) ─────────────
       The punch comes from a fast FM pitch-drop (osc 1 modulates osc 0
       at a high index — this sweeps the pitch from ~180 Hz down to the
       body frequency in the first few ms, giving the "thwack").
       The depth comes from the body sine frequency and decay time.
       kickBoost increases: body level, FM index (more punch transient),
       sub-octave level, and decay time (longer thump).                */
    float bodyHz  = 65.f;                       /* ~C2, classic kick body */
    float bodyAmp = std::min(1.0f, 0.72f * kickBoost);
    float subAmp  = std::min(0.9f, 0.38f * kickBoost);  /* sub octave  */
    float fmIdx   = std::min(14.f, 8.0f * kickBoost);   /* punch transient */
    float bodyDec = 0.08f + 0.10f * (kickBoost - 1.0f); /* longer thump  */
    bodyDec = std::max(0.08f, std::min(0.35f, bodyDec));
    float bodyRel = bodyDec * 1.5f;

    /* osc 0: kick body sine — semi set so noteHz(MIDI24) × ratio = bodyHz
       MIDI 24 = B0 = 30.87 Hz, so semi = 12*log2(bodyHz/30.87) ≈ +12.8
       We use +13 semitones which gives ~65 Hz                         */
    P.osc[0]={W_SIN,13,0, bodyAmp,   0.5f, -1, 0.f, -1};
    /* osc 1: pitch-drop FM operator — 2 octaves up, high index        */
    P.osc[1]={W_SIN,13,0, 0.55f,     0.5f,  0, fmIdx, -1};
    /* osc 2: click noise — very short, gives the initial attack snap   */
    P.osc[2]={W_NOISE,0,0,0.30f,     0.5f, -1, 0.f, -1};
    /* osc 8: sub-octave sine for extra depth                           */
    P.osc[8]={W_SIN, 1,0, subAmp,    0.5f, -1, 0.f, -1};

    /* ── SNARE / HAT (osc 3-7, 9 unchanged) ────────────────────────── */
    P.osc[3]={W_TRI,  0,0, 0.46f,    0.5f, -1, 0.f, -1};  /* snare body  */
    P.osc[4]={W_NOISE,0,0, 0.6f,     0.5f, -1, 0.f,  3};  /* snare noise */
    P.osc[5]={W_NOISE,0,0, 0.55f,    0.5f, -1, 0.f, -1};  /* hat hiss    */
    P.osc[6]={W_SIN,  0,0, 0.20f,    0.5f, -1, 0.f, -1};
    P.osc[7]={W_SIN,  7,0, 0.12f,    0.5f, -1, 0.f, -1};
    P.osc[9]={W_NOISE,0,2, 0.26f,    0.5f, -1, 0.f, -1};

    /* filter: open enough to pass the low kick body                    */
    P.cutoff = std::max(200.f, 6000.f - 1500.f*(kickBoost-1.f));
    P.res=0.28f; P.fenvAmt=0.45f;
    P.keytrack=0.f;
    P.drive = std::min(2.5f, 1.0f + 0.5f*(kickBoost-1.f)); /* saturation*/

    /* amp envelope: fast attack, no sustain, decay = body decay        */
    P.aA=0.001f; P.aD=bodyDec; P.aS=0.0f; P.aR=bodyRel;
    /* filter envelope: slightly longer so the low end blooms           */
...
static int compose(Composer&C,Note*notes,char*report,int length=5,float kickBoost=1.0f)
{
    C.notes=notes; C.n=0;
    ...
    /* length 1-10 scales section bar counts.
       Scale maps 1→0.4, 5→1.0, 10→2.0 (roughly linear in log space)
       so length=5 is identical to the original piece.                 */
    if(length<1)  length=1;
    if(length>10) length=10;
    float sc = 0.4f + (length-1) * (1.6f/9.f);  /* 0.4 .. 2.0        */
    /* helper: scale a fixed bar count, minimum 2 bars                 */
    auto bars=[&](int n)->int{ return std::max(2,(int)roundf(n*sc)); };
    /* random-range bars: scale both endpoints, keep random within them */
    ...
    /* longer pieces get an extra chorus reprise                        */
    ...
    char*p=report;
    char*report_end=report+512;
    p+=snprintf(p,(size_t)(report_end-p),"Key: %s minor   Tempo: %d BPM   Length: %d/10   Kick: %.1fx\nForm: ",
               NOTE_N[C.key%12],C.tempo,length,kickBoost);
    for(int s=0;s<nf;s++) p+=snprintf(p,(size_t)(report_end-p),"%s(%d) ",form[s].name,form[s].bars);
    p+=snprintf(p,(size_t)(report_end-p),"\nVerse prog:  "); for(int i=0;i<4;i++) p+=snprintf(p,(size_t)(report_end-p),"%s ",RN[vProg[i].deg]);
    p+=snprintf(p,(size_t)(report_end-p),"\nChorus prog: "); for(int i=0;i<4;i++) p+=snprintf(p,(size_t)(report_end-p),"%s ",RN[cProg[i].deg]);
    p+=snprintf(p,(size_t)(report_end-p),"\n");
    return C.n;
}
...
/* ══════════════════════════════════════════════════════════════════════
   FREEWAY LAYER
   Synthesises a roadside traffic soundscape and mixes it into the
   already-rendered stereo music buffers L[] and R[].

   traffic  0.0 = no cars at all
            0.5 = light background freeway (a few cars per minute)
            1.0 = moderate rush-hour traffic
            2.0 = heavy congested freeway

   honking  0.0 = no horns
            0.5 = occasional irritated toot
            1.0 = impatient rush-hour honking
            2.0 = gridlock rage

   Every run produces a different set of cars (speeds, lanes, era,
   nation) and horn events seeded from the main RNG.

   Physics:
   • Exact closed-form Doppler: listener at origin, car moves at
     constant speed v along z-axis, closest approach distance d.
     Position x(t) = v*(t - tPass), range r(t) = sqrt(x²+d²).
     Emission time tau solved via one Newton step from r(t)/c guess.
   • Engine: harmonic series with air-absorption rolloff per partial.
     US V8: half-order (0.5×) harmonics for the burble character.
     UK/JP inline-4: full-order harmonics, faster rolloff.
   • Horns: dual-tone (US 350+440 Hz, UK 320+400, JP single 460 Hz),
     brassy harmonic stack, forward-directive (loud approaching,
     muted receding — horns face forward through the grille).
   • Ground reflection: second arrival 0.6 m below road level.
   • Distant wash: low-passed pink-ish noise swell under everything.
   ══════════════════════════════════════════════════════════════════════ */

static float fw_rnd() {  /* uses global g_rng already seeded */
...
struct FWCar {
    float tPass;          /* time of closest approach (s)              */
    float v;              /* speed m/s (positive = left to right)      */
    float d;              /* closest approach distance (m)             */
    float loud;           /* overall gain                              */
    /* engine */
    float fFire;          /* firing frequency at cruise Hz             */
    bool  halfOrder;      /* true = V8 half-order harmonics            */
    float hAmp[14];       /* per-harmonic amplitude                    */
    float roarAmp;        /* broadband engine roar level               */
    float roarHz;         /* roar low-pass corner                      */
    /* horn */
    bool  hasHorn;
    float hornF1, hornF2; /* tone frequencies (F2=0 → single tone)    */
    float hornStart;      /* s before tPass horn begins                */
    float hornDur;        /* duration s                                */
    bool  hornAngry;      /* long lean vs short blips                  */
};

static void fw_buildCar(FWCar&C, float tPass, float dur,
                        float trafficLevel, float honkLevel,
                        bool isTailgater)
{
    C.tPass = tPass;
    /* four virtual lanes: two approaching (positive v) two receding   */
    ...
    /* nation / era */
    float nr = fw_rnd();
    int nation = (nr<0.55f)?0:(nr<0.75f)?2:1; /* 0=US 1=UK 2=JP */
    bool era60 = fw_rnd()<0.25f;
    bool era80 = !era60 && fw_rnd()<0.40f;

    bool isV8 = (nation==0) ? (fw_rnd()<0.78f) : (fw_rnd()<0.08f);
    float rpm = isV8 ? fw_rr(2000.f,2800.f)
                     : (nation==1 ? fw_rr(2800.f,4000.f) : fw_rr(2400.f,3400.f));
    C.fFire    = rpm/60.f * (isV8 ? 4.f : 2.f);  /* firing freq      */
    C.halfOrder= isV8;
    ...
    for(int k=0;k<14;k++){
        ...
        if(isV8 && (k%2==0)) a *= 1.45f;         /* odd half-orders   */
        C.hAmp[k] = a;
    }

    float baseLoud = isV8 ? 1.05f : (nation==1 ? 0.90f : 0.62f);
    if(era60) baseLoud *= 1.28f;
    if(era80){ baseLoud *= 0.60f;
               for(int k=0;k<14;k++) C.hAmp[k] *= expf(-0.13f*(float)(k+1)); }
    ...
    /* horn decision */
    float pHorn = 0.f;
    if(isTailgater)       pHorn = 0.55f * honkLevel;
    else                  pHorn = 0.06f * honkLevel;
    ...
    if(C.hasHorn){
        ...
        if(C.hornAngry){
            C.hornStart = fw_rr(2.5f,5.0f);   /* starts well before pass */
            C.hornDur   = fw_rr(1.2f,2.8f);
        } else {
            C.hornStart = fw_rr(1.0f,3.0f);
            C.hornDur   = fw_rr(0.12f,0.35f);
        }
    }
}

static inline float fw_lp1(float x, float &z, float c){
                          ...
                          int N, float sr, unsigned noiseSeed)
{
    ...
    for(int si=s0; si<s1; si++){
        ...
        /* retarded time: one Newton step */
        float tau = tg - r0/SPEED_C;
        float xtau = C.v*(tau - C.tPass);
        float rtau = sqrtf(xtau*xtau + C.d*C.d);
        tau = tg - rtau/SPEED_C;              /* one more refine */
        xtau = C.v*(tau - C.tPass);
        rtau = sqrtf(xtau*xtau + C.d*C.d);
        ...
        for(int k=0;k<14;k++){
            ...
            float ab = expf(-5.5e-10f*f*f*rtau);   /* air absorption */
            float ph = C.fFire*tau*ratio;
            ph -= floorf(ph);
            eng += C.hAmp[k]*ab*sinf(2.f*(float)M_PI*ph);
        }

        /* engine roar */
        unsigned ri2 = (unsigned)(si);
        float nz = hn(ri2);
        float roar = fw_lp1(nz, zRoar, cRoar) * C.roarAmp * 3.2f;

        /* ground reflection: second arrival from below road */
        ...
        if(C.hasHorn){
            ...
            if(tauRel >= 0.f && tauRel < C.hornDur){
                /* forward directivity: loud approaching, quiet after  */
                float sgnV = (C.v>=0.f)?1.f:-1.f;
                float fw2 = 0.5f*(1.f - (xtau*sgnV)/sqrtf(xtau*xtau+36.f));
                float dir = 0.18f + 0.82f*fw2;
                float envH = fminf(1.f,tauRel/0.006f)
                           * fminf(1.f,(C.hornDur-tauRel)/0.04f);
                float horn=0.f;
        ...
        /* azimuth pan from lateral position at emission time           */
...
{
    /* distant traffic wash: low-passed noise swell                     */
    ...
    for(int i=0;i<N;i++){
        ...
        L[i] += vL*g;
        R[i] += vR*g*0.9f;   /* slight L/R decorrelation               */
    }
}
                           ...
                           unsigned seed2)
{
    ...
    float dur = N/sr;
    /* number of cars: traffic 0→0 cars, 0.5→8, 1→20, 2→50            */
    int nCars = (int)(trafficLevel * trafficLevel * 25.f
                    + trafficLevel * 8.f);
    nCars = std::max(0, std::min(120, nCars));

    printf("Freeway: %d cars, honk=%.1f\n", nCars, honkLevel);

    /* save & restore g_rng so freeway doesn't affect music seed        */
...
int main(int argc,char**argv)
{
    ...
    /* kick boost: 1.0 = normal, 2.0 = twice as loud (clamped to 1.0 vel) */
    float kickBoost=(argc>5)?(float)atof(argv[5]):1.0f;
    if(kickBoost<0.1f)kickBoost=0.1f;
    if(kickBoost>3.0f)kickBoost=3.0f;
    /* traffic 0.0=none 0.5=light 1.0=moderate 2.0=heavy              */
    float trafficLevel=(argc>6)?(float)atof(argv[6]):0.0f;
    if(trafficLevel<0.f)trafficLevel=0.f;
    if(trafficLevel>2.f)trafficLevel=2.f;
    /* honking 0.0=none 0.5=occasional 1.0=normal 2.0=rage            */
    float honkLevel=(argc>7)?(float)atof(argv[7]):0.0f;
    if(honkLevel<0.f)honkLevel=0.f;
    if(honkLevel>2.f)honkLevel=2.f;
    sr_seed(seed); srand(g_rng);   /* srand gets same value as g_rng  */
    seed = g_rng;                  /* capture actual seed for printing */

    printf("═══ GENERATIVE DISCO MACHINE (macOS CPU) ═══\n");
    printf("SEED: %u   LENGTH: %d/10   KICK: %.1fx   TRAFFIC: %.1f   HONK: %.1f\n",
           seed,length,kickBoost,trafficLevel,honkLevel);
    printf("(rerun './discogen out.wav %d %u %d %.1f %.1f %.1f')\n\n",
           sr,seed,length,kickBoost,trafficLevel,honkLevel);
    ...
    /* — freeway layer: mixed directly into stereo bus — */
    if(trafficLevel>0.f || honkLevel>0.f)
        renderFreeway(L,R,N,(float)sr,trafficLevel,honkLevel,seed^0xF0F0F0F0u);
