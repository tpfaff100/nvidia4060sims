...
struct Note { float t, dur, vel; int midi, prevMidi; };
...
static void defaultPatch(Patch&P,Note*notes,int*nNotes)
{
    ...
    for(int r=0;r<2;r++)
      for(int i=0;i<16;i++){
        ...
        notes[n++]={t,dur,(i%8==0)?1.0f:0.8f,seq[i],prev};
        prev=seq[i]; t+=step;
      }
    /* closing held note with full filter bloom                          */
    notes[n++]={t,2.6f,1.0f,36,prev};
    *nNotes=n;
}

static int loadPatch(const char*path,Patch&P,Note*notes,int*nNotes)
{
    FILE*f=fopen(path,"r");
    if(!f)return 0;
    memset(&P,0,sizeof(P));
    ...
    while(fgets(line,sizeof(line),f)){
        ...
        if(!strcmp(cmd,"osc")){
            ...
            float t,d,v;int m;
            if(sscanf(line,"note %f %f %d %f",&t,&d,&m,&v)==4
               &&n<MAX_NOTES){
                notes[n++]={t,d,v,m,prevMidi}; prevMidi=m;
            }
        }
    }
    fclose(f);
...
int main(int argc,char**argv){
    ...
    Patch P; Note*notes=(Note*)calloc(MAX_NOTES,sizeof(Note));
    int nNotes=0;
    if(patchPath&&loadPatch(patchPath,P,notes,&nNotes))
        printf("Loaded patch: %s (%d osc, %d notes)\n",patchPath,P.nOsc,nNotes);
    else{
        if(patchPath)printf("Could not load %s — using demo patch\n",patchPath);
        defaultPatch(P,notes,&nNotes);
        printf("Demo patch: %d osc, %d notes\n",P.nOsc,nNotes);
    }

    printf("GPU Minimoog | osc=%d lfo=%.1fHz cutoff=%.0fHz res=%.2f\n\n",
           P.nOsc,P.lfoRate,P.cutoff,P.res);
    cudaDeviceProp prop;CUDA_CHECK(cudaGetDeviceProperties(&prop,0));
    printf("Device: %s (SM %d.%d)\n\n",prop.name,prop.major,prop.minor);

    /* buffer length from last note                                     */
    float endT=0.f;
    for(int i=0;i<nNotes;i++)
        endT=fmaxf(endT,notes[i].t+notes[i].dur+P.aR);
    endT+=2.5f;
    int N=(int)(endT*sr);

    Patch*dP;Note*dN;float*dBus;
    CUDA_CHECK(cudaMalloc(&dP,sizeof(Patch)));
    CUDA_CHECK(cudaMalloc(&dN,MAX_NOTES*sizeof(Note)));
    CUDA_CHECK(cudaMalloc(&dBus,(size_t)N*4));
    CUDA_CHECK(cudaMemcpy(dP,&P,sizeof(Patch),cudaMemcpyHostToDevice));
    ...
    free(notes);free(mono);free(L);free(R);
