    ...
    for(int si = s0; si < s1; si++){
        ...
        if(gated && env > 1e-6f && f > 1.f && f < 0.45f*sr){
            ...
            if(o.group == G_WIND || o.windAmp > 0.f){
                /* Wind/mechanical noise: hash-based broadband noise     */
                unsigned h = (unsigned)si * 0x9E3779B9u ^ (unsigned)oi*2654435761u;
                h ^= h>>13; h *= 0xC2B2AE35u; h ^= h>>16;
                float n = (float)(int)h * 4.6566129e-10f;
                sample = n * o.windAmp * env;
...
/* ── Disco chord stabs on Manual II (low: flute+string) ─────────────── */
static void discoChordLow(EvBuf&E,float tOn,float tOff,
                          int key,Chord c,float vel)
{
    /* Voiced in low-mid register: octave 2-3 for weight               */
    int ct[4]; chordTones(key,c,1,ct);
    /* push all voices down one octave for low register emphasis        */
    for(int i=0;i<4;i++){
        int midi=ct[i]-12;
        if(midi<36)midi+=12;
        E.add(tOn,tOff,vel,        midiHz(midi),G_FLUTE);
        E.add(tOn,tOff,vel*0.70f,  midiHz(midi),G_STRING);
        E.add(tOn,tOff,vel*0.50f,  midiHz(midi),G_CELESTE);
    }
    /* also the root an octave lower for sub-bass bed                   */
    int subRoot=ct[0]-24;
    while(subRoot<24)subRoot+=12;
    E.add(tOn,tOff,vel*0.85f, midiHz(subRoot),G_FLUTE);
}
                        ...
                        int dev,float vel,bool withMixture)
{
    ...
    for(int i=0;i<motifN;i++){
        ...
        if(ton<0.f)ton=0.f;
        if(toff>ton+0.01f){
            /* HIGH register: keep melody above tenor break             */
            while(midi<60)midi+=12;
            while(midi>88)midi-=12;
            E.add(ton,toff,vel*rr(0.88f,1.f), midiHz(midi),G_PRINCIPAL);
            E.add(ton,toff,vel*0.65f,          midiHz(midi),G_REED);
            if(withMixture)
                E.add(ton,toff,vel*0.50f,      midiHz(midi),G_MIXTURE);
        }
    }
}

/* ── Counter-melody (low register, flute): inverted a 6th below ─────── */
...
{
    for(int i=0;i<motifN;i++){
        int d=-motifDeg[i]+2;          /* inversion                    */
        int midi=key+MINOR[((d%7)+7)%7]+48; /* low register C3 area   */
        midi=snapChord(key,ch,midi,36,60);
        float ton=bb+motifT[i]*beat;
        float toff=ton+motifD[i]*beat;
        if(ton<0.f)ton=0.f;
        if(toff>ton+0.01f)
            E.add(ton,toff,vel*0.60f,midiHz(midi),G_FLUTE);
    }
}
...
static void composeSong(EvBuf&E,int key,float beat,int length,char*report)
{
    ...
    /* HIGH register for melody (principal+reed): C5-C7 range          */
    int melBase=key+48;
    if(melBase<72)melBase+=12;
    if(melBase>84)melBase-=12;
    ...
    for(int s=0;s<nf;s++){
        ...
        for(int b=0;b<form[s].bars;b++){
            ...
            /* ── ROOM: always feed reverb tail ─────────────────────── */
            int ct4[4];chordTones(key,ch,1,ct4);
            /* extra-heavy room feed for church reverb character        */
            E.add(bb,be+1.0f,E2*0.55f,midiHz(ct4[0]),G_ROOM);
            E.add(bb,be+1.0f,E2*0.40f,midiHz(ct4[2]),G_ROOM);
            E.add(bb,be+1.0f,E2*0.30f,1.f,G_WIND);
            ...
            if(!isIntro||b>=form[s].bars/2){
                if(b%2==0&&!isBreak){
                    ...
                    if(ph==3)dev=4;
                    int reg=melBase;
                    if(isChorus)reg+=5;      /* chorus: even higher    */
                    if(isPre)   reg+=b/2;    /* crescendo upward       */
    ...
    /* Final chord: full organ, very long — let the church reverb ring  */
    float endB=bar*4.f*beat;
    int ct[4];Chord tonic={0,0};chordTones(key,tonic,1,ct);
    /* low manual: full chord held                                      */
    for(int v=0;v<4;v++){
        int lo=ct[v]-12;while(lo<36)lo+=12;
        E.add(endB,endB+14.f,0.90f,midiHz(lo), G_FLUTE);
        E.add(endB,endB+14.f,0.75f,midiHz(lo), G_STRING);
    }
    /* high manual: root position triad                                 */
    for(int v=0;v<3;v++)
        E.add(endB,endB+14.f,0.85f,midiHz(ct[v]+12),G_PRINCIPAL);
    /* pedal: sub-bass and root                                         */
    E.add(endB,endB+14.f,0.95f,midiHz(ct[0]-12),G_PEDAL);
    E.add(endB,endB+14.f,0.90f,midiHz(ct[0]),   G_PEDAL);
    /* saturate the room modes for a big reverb tail                    */
    E.add(endB,endB+16.f,1.00f,midiHz(ct[0]),   G_ROOM);
    E.add(endB,endB+16.f,0.80f,1.f,             G_WIND);
...
static void buildOscillators(OscDesc*D, float roomScale)
{
    ...
    /* ── PEDAL division (1200 osc) ──────────────────────────────────
       ...
       16' at low C = 32.7 Hz (near infrasonic).
       Huge air columns, very slow speech, few harmonics (lower partial
       energy falls inside the pipe chest before radiating).
       150 voices × 4 harmonics = 600 per rank × 2 = 1200.            */
    {
        float pedalRatios[2]={0.5f,1.f};   /* 16'=0.5× vs 8'=1× of note */
        float pedalVols[2]  ={1.20f,0.80f};
        for(int rank=0;rank<2;rank++){
            for(int v=0;v<150;v++){
                float det=rr(-1.5f,1.5f);
                float vPan=rr(-0.35f,0.35f);  /* pedal pipes near centre */
                for(int h=1;h<=4;h++){
                    OscDesc&d=D[idx++];
                    d.freqRatio  =pedalRatios[rank]*(float)h;
                    d.harmonic   =h;
                    d.detuneCents=det;
                    d.inharmonB  =0.00008f;    /* large open pipe, low B */
                    /* pedal: fundamental dominant, very few harmonics   */
                    d.amp=pedalVols[rank]*powf(1.f/(float)h,1.2f)*0.014f
                          *(0.85f+0.30f*rnd());
                    d.phase0=rnd();
                    pan(vPan,d.panL,d.panR);
                    /* Slow speech: large air column takes time to start */
                    d.aA=0.055f;d.aD=0.10f;d.aS=0.97f;d.aR=0.35f;
                    d.chiffDur   =(h==1)?rr(0.035f,0.055f):0.f;
                    d.chiffBandHz=(h==1)?rr(200.f,400.f):0.f;
                    d.tremPitchDepth=rr(1.f,3.f);  /* pedal: slight trem */
                    d.tremAmpDepth  =rr(0.02f,0.05f);
                    d.tremPhase=rnd();
                    d.windAmp=0.f;d.stoppedPipe=0;d.roomDecay=0.f;
                    d.group=G_PEDAL;
                }
    ...
    /* ── WIND / mechanical noise (600 osc) ──────────────────────────
       Wind chest rumble, blower hum, key action, wood creak.
       Not pitch-locked: these are noise sources at low frequencies.   */
    {
        for(int i=0;i<600;i++){
            OscDesc&d=D[idx++];
            /* wind noise: low-frequency random */
            d.freqRatio   =rr(20.f,180.f);   /* absolute Hz           */
            d.harmonic    =1;
            d.detuneCents =0.f;
            d.inharmonB   =0.f;
            d.amp         =0.00015f*(0.4f+0.8f*rnd());
            d.phase0=rnd();
            pan(rr(-0.4f,0.4f),d.panL,d.panR);
            d.aA=1.0f;d.aD=0.f;d.aS=1.0f;d.aR=2.0f;
            d.chiffDur=0.f;d.chiffBandHz=0.f;
            d.tremPitchDepth=0.f;d.tremAmpDepth=0.f;d.tremPhase=0.f;
            d.windAmp=0.0004f*(0.5f+0.8f*rnd());
...
static float* buildIR(int irLen, float sr, float roomScale, unsigned seed2)
{
    ...
    /* The tail is now scaled relative to the direct impulse.
       Apply a gentle overall scale so the reverb sits at ~30% of dry. */
    for(int i=0;i<irLen;i++) ir[i]*=0.30f;
...
int main(int argc,char**argv)
{
    ...
    printf("Rendering: %d blocks × %d threads, %d chunks\n",
    ...
    fflush(stdout);
    {
        ...
        clock_t t0=clock();
        applyConvReverb(hL,hR,totalSamples,ir,irLen,0.65f,1.0f);
        clock_t t1=clock();
