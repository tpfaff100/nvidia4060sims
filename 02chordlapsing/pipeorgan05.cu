...
/* renderAdaptiveFreeway — wraps renderFreeway with per-bar dynamic shaping.
   Analyses the organ's L channel to derive a time-varying traffic level:
     base level = trafficBase  (grows over intro bars)
     each bar:   organRMS computed over bar duration
                 trafficMult = 1 + (1 - organRMS/peakRMS) * trafficDynamic
   So quiet organ bars get up to 2× the base traffic level.
   All traffic rendered into separate tL/tR buffers then blended.         */
static void renderAdaptiveFreeway(
    float*L, float*R, int N, float sr,
    float trafficBase, float honkLevel,
    float trafficDynamic,   /* 0=flat level, 1=full inverse-dynamic swing */
    unsigned seed2, float beatDur)
{
    if(trafficBase<=0.f&&honkLevel<=0.f) return;

    /* analyse organ L channel: compute RMS in sliding bars              */
    ...
    /* build per-sample traffic gain envelope                            */
    float*tGain = (float*)malloc(N*sizeof(float));
    /* intro ramp: traffic starts at 0 and fades in over first 8 bars   */
    for(int b=0;b<nBars;b++){
        float introFade = fminf(1.f, (float)(b+1)/8.f);  /* 0→1 over 8 bars */
        float organFrac = barRMS[b] / peakRMS;            /* 0=quiet, 1=loud  */
        float dynBoost  = 1.f + (1.f - organFrac) * trafficDynamic;
        float gain      = trafficBase * introFade * dynBoost;
        /* smooth the gain: write into barSamples samples                */
        int s0=b*barSamples, s1=std::min(N,s0+barSamples);
        for(int i=s0;i<s1;i++) tGain[i]=gain;
    }
    /* one-pole smooth the gain envelope (200ms time constant)           */
    ...
    printf("Traffic: base=%.2f honk=%.2f dynamic-swing=%.1f\n",
           trafficBase, honkLevel, trafficDynamic);
}
...
int main(int argc,char**argv)
{
    ...
    if(trafficLevel>0.f||honkLevel>0.f){
        ...
        fflush(stdout);
        /* trafficDynamic=1.0: full inverse swing (quiet organ = 2x traffic) */
