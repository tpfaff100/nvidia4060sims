 ...
 *
 * RUN:
 *   musetraffic01 [out.wav] [sr] [seed] [length] [room] [tremulant] [traffic] [honk]
 *
 *   pipeorgan01                                new piece, cathedral room, no traffic
 *   pipeorgan01 out.wav 48000 0 5 2.0 0.6 1.0 0.8   cathedral + moderate traffic
 *   pipeorgan01 out.wav 48000 0 5 3.0 0.4 1.5 1.5   vast nave + heavy traffic + honking
 *   pipeorgan01 out.wav 48000 0 5 5.0 0.2 2.0 2.0   Hagia Sophia + gridlock rage
 *
 * PARAMETERS
 *   seed        0 = different every run (default), else reproducible
 *   length      1=~1:15   5=~3:00   10=~6:15
 *   room        0=dry chamber  1.0=large cathedral  2.0=gothic basilica  [1.0]
 *               3.0=vast nave  5.0=Hagia Sophia scale (RT60~11s, ~2min conv)
                          ...
                          int key,Chord c,float vel)
{
    ...
    /* root voicing: C1-C2 range (midi 24-36) — maximum room excitation.
       NOTE: no celeste down here — real organs never build celeste ranks
       below tenor C because the detuned beating (0.2-0.5 Hz at these
       frequencies) turns to muddy wobble instead of shimmer.           */
    for(int i=0;i<4;i++){
        int midi=ct[i];
        while(midi>40)midi-=12;      /* pull into C1-C2 band           */
        while(midi<24)midi+=12;
        E.add(tOn,tOff,vel,          midiHz(midi),   G_FLUTE);
        E.add(tOn,tOff,vel*0.55f,    midiHz(midi),   G_STRING);
        /* octave double: celeste is safe up here (C2-C3)               */
        E.add(tOn,tOff,vel*0.50f,    midiHz(midi+12),G_FLUTE);
        E.add(tOn,tOff,vel*0.35f,    midiHz(midi+12),G_CELESTE);
    }
        ...
        for(int b=0;b<form[s].bars;b++){
            ...
            if(!isIntro||b>=form[s].bars/2){
                ...
                if(!isBreak){
                    ...
                    /* one moderate sub-bass room feed per bar — the
                       reverb IR provides the ring; stacking many long
                       G_ROOM events here causes LF mud                 */
                    int subPitch=root-12; while(subPitch<12)subPitch+=12;
                    E.add(bb,be+0.5f,0.45f,midiHz(subPitch),   G_ROOM);
                    E.add(bb,be+0.5f,0.30f,midiHz(subPitch+12),G_ROOM);
                }else{
            ...
            E.add(bb,be+0.8f,E2*0.45f,midiHz(deepRoot),   G_ROOM);
            E.add(bb,be+0.8f,E2*0.30f,midiHz(deepRoot+7), G_ROOM);
...
static void buildOscillators(OscDesc*D, float roomScale)
{
        ...
        for(int rank=0;rank<2;rank++){
            for(int v=0;v<150;v++){
                ...
                for(int h=1;h<=4;h++){
                    ...
                    d.phase0=rnd();
                    pan(vPan,d.panL,d.panR);
                    /* Slow speech, moderate release: rings into the next
                       beat without stacking 6 notes deep               */
                    d.aA=0.065f;d.aD=0.18f;d.aS=0.97f;
                    d.aR=rr(0.55f,0.80f);  /* ~2 notes overlap max      */
       ...
       Each mode has its own RT60-based amplitude decay.               */
    {
        ...
        float rt60=rr(2.5f+1.8f*roomScale, 4.0f+2.2f*roomScale);
...
static float* buildIR(int irLen, float sr, float roomScale, unsigned seed2)
{
    ...
    /* Room dimensions scale with roomScale:
       roomScale 1 = large cathedral (e.g. York Minster)
       roomScale 2 = gothic basilica (e.g. Cologne Cathedral)
       roomScale 5 = vast historic space (e.g. Hagia Sophia, St Paul's)  */
    ...
    /* RT60: 2.5s at roomScale=0.5, 11.5s at roomScale=5
       (Hagia Sophia sub-bass measured ~11s; Gothic cathedrals 7-9s)  */
    float rt60 = 2.5f + roomScale * 1.8f;
    ...
    for(int i=0;i<irLen;i++) ir[i]*=0.42f;
...
/* ══════════════════════════════════════════════════════════════════════
   FREEWAY TRAFFIC LAYER
   Physically-modelled Doppler traffic mixed inversely with organ amplitude.
   When the organ is loud, traffic is quiet; when the organ fades or rests,
   traffic swells — simulating the experience of hearing a cathedral organ
   from the steps outside, with the city audible in the quieter passages.
   Functions ported directly from musetraffic01.cu (same physics, same RNG
   save/restore so traffic does not disturb the organ seed).
   ══════════════════════════════════════════════════════════════════════ */
                        ...
                        float trafficLevel,float honkLevel,bool isTailgater)
{
    ...
    float pHorn=isTailgater?0.85f*honkLevel:0.25f*honkLevel;
    C.hasHorn=(fw_rnd()<fminf(0.98f,pHorn));
    if(C.hasHorn){
        ...
        C.hornAngry=(fw_rnd()<0.55f*honkLevel);
        if(C.hornAngry){C.hornStart=fw_rr(3.0f,6.0f);C.hornDur=fw_rr(1.5f,3.5f);}
        else           {C.hornStart=fw_rr(0.5f,2.0f);C.hornDur=fw_rr(0.3f,0.8f);}
    }
}
...
/* renderAdaptiveFreeway — wraps renderFreeway with per-bar dynamic shaping.
   Renders traffic into SEPARATE tL/tR buffers, peak-normalises them
   independently of the organ, then blends at an explicit dB ratio so
   writeWav's peak normalisation cannot crush the traffic to inaudibility.
   trafficMix: traffic peak level relative to organ peak (0.3 = -10 dB).  */
static void renderAdaptiveFreeway(
    float*L, float*R, int N, float sr,
    float trafficBase, float honkLevel,
    float trafficDynamic,
    float trafficMix,       /* traffic peak rel to organ: 0.25=-12dB 0.5=-6dB */
    unsigned seed2, float beatDur)
{
    if(trafficBase<=0.f&&honkLevel<=0.f) return;

    /* analyse organ L channel: RMS per bar                             */
    int barSamples = (int)(beatDur * 4.f * sr);
    if(barSamples<1) barSamples=1;
    int nBars = (N + barSamples - 1) / barSamples;

    float peakRMS = 1e-9f;
    float*barRMS  = (float*)calloc(nBars, sizeof(float));
    for(int b=0;b<nBars;b++){
        int s0=b*barSamples, s1=std::min(N,s0+barSamples);
        float rms=0.f;
        for(int i=s0;i<s1;i++) rms+=L[i]*L[i];
        rms=sqrtf(rms/(float)(s1-s0));
        barRMS[b]=rms;
        peakRMS=fmaxf(peakRMS,rms);
    }

    /* per-bar gain: intro ramp + inverse organ dynamic                 */
    float*tGain = (float*)malloc(N*sizeof(float));
    for(int b=0;b<nBars;b++){
        float introFade = fminf(1.f, (float)(b+1)/8.f);
        float organFrac = barRMS[b] / peakRMS;
        float dynBoost  = 1.f + (1.f - organFrac) * trafficDynamic;
        float gain      = trafficBase * introFade * dynBoost;
        int s0=b*barSamples, s1=std::min(N,s0+barSamples);
        for(int i=s0;i<s1;i++) tGain[i]=gain;
    }
    /* smooth gain envelope (200ms)                                     */
    float smoothC = expf(-1.f/(0.2f*sr));
    float gSmooth=tGain[0];
    for(int i=0;i<N;i++){gSmooth=gSmooth*smoothC+tGain[i]*(1.f-smoothC);tGain[i]=gSmooth;}

    float maxGain=0.f;
    for(int i=0;i<N;i++) maxGain=fmaxf(maxGain,tGain[i]);

    /* render traffic into its own stereo buffers                       */
    float*tL=(float*)calloc(N,sizeof(float));
    float*tR=(float*)calloc(N,sizeof(float));
    float honkEff = honkLevel * 1.8f;  /* amplify honk for audibility  */
    renderFreeway(tL,tR,N,sr,maxGain,honkEff,seed2);

    /* apply time-varying gain shape to traffic buffers                 */
    for(int i=0;i<N;i++){
        float g = (maxGain>1e-6f) ? (tGain[i]/maxGain) : 0.f;
        tL[i]*=g; tR[i]*=g;
    }

    /* peak-normalise traffic independently of organ                    */
    float tPeak=1e-9f;
    for(int i=0;i<N;i++){tPeak=fmaxf(tPeak,fabsf(tL[i]));tPeak=fmaxf(tPeak,fabsf(tR[i]));}

    /* find organ peak for ratio calculation                            */
    float oPeak=1e-9f;
    for(int i=0;i<N;i++){oPeak=fmaxf(oPeak,fabsf(L[i]));oPeak=fmaxf(oPeak,fabsf(R[i]));}

    /* scale traffic so its peak = trafficMix * organPeak              */
    float tScale = (oPeak * trafficMix) / tPeak;
    printf("  Traffic peak: %.4f  Organ peak: %.4f  Scale: %.2fx  Mix: %.2f (%.1f dB)\n",
           tPeak, oPeak, tScale, trafficMix, 20.f*log10f(trafficMix));
    fflush(stdout);
    ...
    free(barRMS); free(tGain); free(tL); free(tR);
    printf("Traffic: base=%.2f honk=%.2f dynamic=%.1f mix=%.2f\n",
           trafficBase, honkLevel, trafficDynamic, trafficMix);
}

/* ── Sub-sonic high-pass: 2nd-order Butterworth at 22 Hz ─────────────
   Removes infrasonic energy below the 16' pipe fundamental (32.7 Hz)
   that would otherwise eat headroom and cause pumping/mud.  Unlike a
   saturator this is perfectly linear — no intermodulation distortion. */
static void subsonicHPF(float*x,int N,float sr){
    float fc=22.f;
    float w=tanf((float)M_PI*fc/sr);
    float w2=w*w;
    float q=0.7071f;                    /* Butterworth                  */
    float a0=1.f+w/q+w2;
    float b0= 1.f/a0, b1=-2.f/a0, b2=1.f/a0;
    float a1=2.f*(w2-1.f)/a0, a2=(1.f-w/q+w2)/a0;
...
int main(int argc,char**argv)
{
    ...
    if(roomScale>5.f)roomScale=5.f;  /* 5=Hagia Sophia scale, RT60~11s */
    float tremulant  =(argc>6)?(float)atof(argv[6]):0.6f;
    if(tremulant<0.f)tremulant=0.f;
    if(tremulant>1.f)tremulant=1.f;
    /* traffic: base density 0-2 (scales car count)                    */
    float trafficLevel=(argc>7)?(float)atof(argv[7]):0.0f;
    if(trafficLevel<0.f)trafficLevel=0.f;
    if(trafficLevel>2.f)trafficLevel=2.f;
    /* honk: horn probability 0-2 (0=none, 2=rage)                     */
    float honkLevel  =(argc>8)?(float)atof(argv[8]):0.0f;
    ...
    printf("SEED: %u  LENGTH: %d/10  ROOM: %.1f  TREMULANT: %.2f  TRAFFIC: %.1f  HONK: %.1f\n",
           seed,length,roomScale,tremulant,trafficLevel,honkLevel);
    printf("Reproduce: pipeorgan01 out.wav %d %u %d %.1f %.2f %.1f %.1f\n\n",
           sr,seed,length,roomScale,tremulant,trafficLevel,honkLevel);
    fflush(stdout);
    ...
    fflush(stdout);
    {
        /* IR length: 1.5s at roomScale=0, up to 12s at roomScale=5
           (Hagia Sophia sub-bass RT60 ~11s).  Convolution time scales
           linearly — budget ~2 min CPU at roomScale=5, 3-min song.   */
        int irLen=(int)(fminf(roomScale*2.0f+1.5f, 12.0f)*(float)sr);
        ...
        clock_t t0=clock();
        applyConvReverb(hL,hR,totalSamples,ir,irLen,0.75f,1.0f);
        clock_t t1=clock();
    ...
    #endif
    printf("Post: sub-sonic filter (22 Hz HPF)...\n");fflush(stdout);
    subsonicHPF(hL,totalSamples,(float)sr);
    subsonicHPF(hR,totalSamples,(float)sr);

    /* — Adaptive traffic layer ————————————————————————————————————————
       Rendered after reverb so the organ's wet signal drives the
       envelope analysis.  Traffic swells into quiet passages and fades
       under loud organ sections.  Starts silent, ramps over 8 bars.   */
    if(trafficLevel>0.f||honkLevel>0.f){
        printf("Post: adaptive traffic layer (traffic=%.1f honk=%.1f)...\n",
               trafficLevel,honkLevel);
        fflush(stdout);
        /* trafficMix=0.25: traffic peaks at 25% of organ peak = -12 dB. */
        renderAdaptiveFreeway(hL,hR,totalSamples,(float)sr,
                              trafficLevel,honkLevel,
                              1.0f,              /* full dynamic swing    */
                              0.25f,             /* traffic at -12 dB     */
                              seed^0xF0F0F0F0u,
                              beat);
        /* second HPF pass: traffic itself adds some sub-sonic content  */
        subsonicHPF(hL,totalSamples,(float)sr);
        subsonicHPF(hR,totalSamples,(float)sr);
    }
